...
module Consts =
    ...
    let [<Literal>] BALL_X_FORCE = 1.0
    let [<Literal>] BALL_Y_FORCE = -2.5
    let [<Literal>] BALL_X_START = 100.
...
let init () =
    ...
    let balls = [| ball 1 BALL_X_FORCE 0. BALL_X_START |]
    let walls = [|
...
let moveHorizontally (body: Matter.Body) (x : float) =
    matter.Body.applyForce(body, body.position, vector x 0.)

let onCollision (engine: Matter.Engine) handler =
